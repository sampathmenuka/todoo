...
const signup = async(req, resp)=>{
    try{
        ...
        if(existingUser) return resp.status(409).json({'massage':"User alredy exists"});

        const hashedPassword = await bcrypt.hash(password,10);
        const savedUser = await User.create({
            name,email,passwordHash:hashedPassword
        });
        resp.status(201).json({'massage':"User created Succesfully",userId:savedUser._id});
    }catch (e){
        resp.status(500).json({'massage':'Signup error',error:e});
    }
}
...
const login = async(req, resp)=>{
    ...
    resp.status(500).json({'massage':'login error',error:e});
    }
